...
impl VectorReader {
    ...
    ///
    /// **Binary-safe:** this returns the raw bytes with no UTF-8 validation, so
    /// it round-trips arbitrary binary content (e.g. ISO-WKB). (Upstream routed
    /// this through the UTF-8-validating string reader and silently returned
    /// empty bytes for non-UTF-8 blobs; the vendored copy fixes that.)
    ///
    /// # Safety
    ///
    /// - `idx` must be less than [`row_count`][Self::row_count].
    ...
    pub unsafe fn read_blob(&self, idx: usize) -> &[u8] {
        // SAFETY: BLOB uses the same duckdb_string_t layout as VARCHAR; the
        // caller guarantees `self.data` is a BLOB vector and `idx` is in bounds.
