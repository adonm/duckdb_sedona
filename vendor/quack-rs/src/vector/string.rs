...
/// Reads a `DuckDB` `BLOB` (binary) value from a raw vector data pointer at the
/// given row index, returning the **raw bytes** without any UTF-8 validation.
///
/// `DuckDB` stores `BLOB` in the same 16-byte `duckdb_string_t` layout as
/// `VARCHAR` (inline for ≤ 12 bytes, pointer otherwise). This is the binary-safe
/// counterpart of [`read_duck_string`]: unlike that function (which validates
/// UTF-8 and substitutes an empty string on failure), this never inspects the
/// bytes, so it round-trips arbitrary binary content — including ISO-WKB whose
/// IEEE-754 coordinate bytes are frequently invalid UTF-8.
///
/// Returns an empty slice only when the internal pointer (for long blobs) is null.
///
/// # Safety
///
...
pub unsafe fn read_duck_blob<'a>(data: *const u8, idx: usize) -> &'a [u8] {
    // SAFETY: each duckdb_string_t is exactly DUCK_STRING_SIZE bytes; the caller
    // guarantees `data` is valid and `idx` is in bounds.
    let str_ptr = unsafe { data.add(idx * DUCK_STRING_SIZE) };
    let raw_bytes: &'a [u8; DUCK_STRING_SIZE] =
        unsafe { &*str_ptr.cast::<[u8; DUCK_STRING_SIZE]>() };
